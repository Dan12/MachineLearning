...
public class LogisticRegression {
    ...
    public static Matrix sigmoid(Matrix z){
        return MTJExt.divideExtend(MTJExt.single(1), MTJExt.plusExtend(MTJExt.single(1), MTJExt.powExtend(MTJExt.single(Math.E), z.scale(-1))));
    }
    ...
    public double costFunction(){
        ...
        double sumPos = (((tempY.scale(-1)).transpose(new DenseMatrix(1,m))).mult(MTJExt.logExtend(sigmoid(X.mult(Theta, new DenseMatrix(m,1)))), new DenseMatrix(1,1))).get(0,0);
        double sumNeg = (((MTJExt.minusExtend(MTJExt.single(1), y)).transpose(new DenseMatrix(1,m))).mult(MTJExt.logExtend(MTJExt.minusExtend(MTJExt.single(1), sigmoid(X.mult(Theta, new DenseMatrix(m,1))))), new DenseMatrix(1,1))).get(0,0);
    ...
    public Matrix gradients(double alpha){
        Matrix temp = (X.transpose(new DenseMatrix(n,m))).mult(MTJExt.minusExtend(sigmoid(X.mult(Theta, new DenseMatrix(m,1))),y), new DenseMatrix(n,1));
